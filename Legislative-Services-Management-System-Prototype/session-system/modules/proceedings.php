...
<div class="card">
  ...
  <div style="padding:12px 14px;border:1px dashed var(--line);border-radius:12px;margin-bottom:16px;background:var(--paper-alt);">
    ...
    <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      ...
      <span id="rec-status" style="font-size:12.5px;color:var(--ink-600);">Pick your spoken language above, then speak what's happening — it types straight into the note below as you talk.</span>
    </div>
  </div>
...
// Real-time, word-by-word auto-type, same as before — but powered entirely
// by the browser (Web Speech API / SpeechRecognition), with NO server,
// NO Python, and NO local service to install or keep running. This works
// on any host, including free PHP hosting, because nothing leaves the
// browser except the page itself.
//
// Trade-off vs. the Whisper-based version: this engine locks to ONE
// language per recording session (pick English or Tagalog/Filipino from
// the dropdown above before you start) — it can't auto-detect a language
// switch mid-sentence the way Whisper could. Supported in Chrome and Edge;
// Firefox/Safari support is limited or absent.
...
function ssmsToggleRecording() {
  ...
  if (!SpeechRecognitionCtor) {
    status.textContent = 'Voice recording isn\'t supported in this browser. Please use Chrome or Edge, or type your entry directly below.';
    return;
  }
  ...
  if (!ssmsRecording) {
    ...
    ssmsRecognition.onerror = (event) => {
      if (event.error === 'not-allowed' || event.error === 'permission-denied') {
        status.textContent = 'Could not access the microphone. Please allow microphone access and try again.';
        ssmsStopRecordingUI();
      } else if (event.error === 'no-speech') {
        // Silence is normal mid-recording — the engine will auto-restart via onend below.
      } else {
    ...
    // The browser engine stops itself after a pause in speech. If the user
    // hasn't clicked Stop, restart it automatically so recording feels
    // continuous, same as before.
    ssmsRecognition.onend = () => {
      if (ssmsRecording) {
        try { ssmsRecognition.start(); } catch (err) { /* already starting, ignore */ }
      }
    };
    ...
    try {
      ...
      status.textContent = 'Listening — click "Stop & Transcribe" when done.';
    } catch (err) {
